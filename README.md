...
플레이어는 다양한 성격의 상대와의 상호작용을 통해 자신의 의사소통 방식과 감정 표현이 어떤 영향을 미치는지를 자연스럽게 인식하게 되며,
이를 통해 보다 깊은 자기 이해와 타인에 대한 공감 능력을 키울 수 있는 계기가 되기를 기대합니다.
...
```python
  ...
  "first_meeting_behavior": "ISTJ 유형은 첫 만남에서 조심스럽고 신중한 태도를 보입니다. 감정을 쉽게 드러내기보다는 상황을 관찰하며, 상대방이 신뢰할 만한 사람인지 판단하려고 합니다. 말을 아끼는 편이지만, 필요할 때는 논리적이고 분명한 의견을 전달합니다.",
...
```
### 3. `app.py`(streamlit 구현 코드 및 prompt)
```python
...
1. 1. 사용자의 입력에 따라 지정된 MBTI 캐릭터 설정에 기반하여 자연스럽게 한국어로 대답하세요. 
2. 같은 입력이어도 MBTI에 따라 대답과 느끼는 감정이 달라질 수 있습니다. 이 점을 유의해서 대답하세요.
3. 감정을 먼저 상상한 후, 인간처럼 표현은 1~2문장으로 짧게 말하세요.
